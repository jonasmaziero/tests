...
call rng_init(optg)  ! Initializes the RNG
...
call rng(optg, ns,rn) ;   call rng_unif(optg, ns, xmin, xmax, rnu) 
call rng_gauss(optg, ns, rnga) ;   call rng_exp(optg, ns, rne)
...
! plot 'rn_2d.dat' 
! splot 'rn_3d.dat'
call rng(optg, 10**4, rn) ;   do j = 1, 10**4, 2 ;   write(12,*) rn(j), rn(j+1) ;   enddo  ! For the 2D scatter plot
close(12)
...
!------------------------
 subroutine rng_corr(optg)  ! Computes some averages, moments, and correlations and write them on a file
implicit none
...
! Gnuplot commands to see the results
! plot 'rn_corr.dat' u 1:2 w lp t 'avg'
...
implicit none
integer, parameter :: d = 10  ! Dimension of the unitary matrices
...
call rug(optg, d, ru) ;   ip_jj = 0.d0 ;   ip_jk = 0.d0 
...
call rug(optg, d, ru) ;   call adjoint(d, d, ru, rud)
...
open(unit = 11, file = 'ru_egval_circle.dat', status = 'unknown')  ! Gnuplot commands to see the results: plot 'ru_egval_circle.dat'
j = -d+1
do
  j = j + d ;   if ( j > 10**3 ) exit ;   call rug(optg, d, ru) ;   call lapack_zgeev('N', d, ru, egv) 
...
doj: do j = 1, ns
  call rug(optg, d, ru) ;   call lapack_zgeev('N', d, ru, egv)  ! Gets the random unitary and its eigenvalues
    !write(*,*) 'j = ', j
  ! Puts the eigenphases in [0,2*pi] and sort them
  forall ( m = 1:d ) egp(m) = dble(-(0.d0,1.d0)*log(egv(m))) + pi ;   call qsort(egp, d)
  ! Normalize the spacings by the average spacing
  forall ( m = 1:(d-1) ) sp(m) = egp(m+1)-egp(m) ;   asp = sum(sp)/dble(d-1) ;   sp = sp/asp
  ! Counter for the different eigenvalues and spacings
  dok: do k = 1, d 
    ! Counter for finding where a certain eigenvalues or spacing is in the whole range
    dol: do l = 1, ni  
      ! Density of the eigenvalues
      if ( (egp(k) >= ((dble(l)-1.d0)*delta)) .and. (egp(k) < (dble(l)*delta)) ) ct(l) = ct(l) + 1   
      ! Density of the eigenvalues spacings 
      if1: if ( k < d ) then
        if ( (sp(k) >= ((dble(l)-1.d0)*delta)) .and. (sp(k) < (dble(l)*delta)) ) cts(l) = cts(l) + 1              
      endif if1
    enddo dol
  enddo dok
enddo doj
...
write(*,*) 'Sum egval counting:', dble(sum(ct))/dble(ns*d), ', Sum egval spacing counting:', dble(sum(cts))/dble(ns*(d-1))
! Write the densities in a file
do l = 1, ni ;   write(12,*) (dble(l)-0.5)*delta, dble(ct(l))/dble(ns*d), dble(cts(l))/dble(ns*(d-1)) ;   enddo  
close(12)
...
do j = 1, 5 ;   d = 2**j 
 allocate( rsv(1:d) )
 call rsvg(optg, d, rsv) ;  write(*,*) "No. qubits", j, "norm", norm(d, rsv) 
 deallocate(rsv)
enddo
...
do  d = 2, 40, 2
  ...
  do j = 1, ns ;   call rsvg(optg, d, rsv)
  call rsvg(optg, d, rsv2) ;   avgf = avgf + fidelity_pp(d, rsv, rsv2) ;   enddo
...
! Gnuplot commands to see the results: plot 'rsv_EE.dat' w lp
do d = 2, 25, 2  ! Changes of dimension of the RSV, which is = d*d
  allocate( rsv(1:d*d), rho(1:d*d,1:d*d), rhoa(1:d,1:d) ) ;   avg_ent = 0.d0
  do j = 1, ns
    call rsvg(optg, d*d, rsv) ;   call projector(rsv, d*d, rho)
    call partial_trace_b_he(rho, d, d, rhoa) ;   avg_ent = avg_ent + neumann(d, rhoa)
  enddo
...
do N = 1, 10**2, 10**1  ! Changes the no. of RSVs used in the mixture
  ...
  do k = 1, ns
    rho = 0.d0
    do j = 1, N ;   call rsvg(optg, d, rsv) ;   call projector(rsv, d, proj) ;  rho = rho + proj
    enddo
...
do j = 1, 5 ;   d = 2**j 
 allocate( rdm(1:d,1:d), egv(1:d) ) ;   call rdmg(optg, d, rdm)
...
do  d = 4, 20, 2 ;   db = d/da 
  ...
  ct = 0
  do j = 1, ns
    call rdmg(optg, d, rdm)
    call partial_transpose_b(da, db, rdm, rdm_pt) ;   call lapack_zheevd('N', d, rdm_pt, egv)
...
! Gnuplot commands to see the results: plot 'rdm_coh.dat' u 1:2 w lp
do  d = 2, 30, 2 ;   allocate( rdm(1:d,1:d) ) ;   avg_coh = 0.d0
  do j = 1, ns
    call rdmg(optg, d, rdm)
    !avg_coh = avg_coh + coh_l1n(d, rdm)  ! Uncomment this line to compute the L1-norm coherence
    avg_coh = avg_coh + coh_re(d, rdm)  ! Uncomment this line to compute the relative entropy of coherence
  enddo
  ! Uncomment the next line when computing the L1-norm coherence
...
open(unit = 12, file = 'rdm_fid.dat', status = 'unknown')
! Gnuplot commands to see the results: plot 1/x w l t 'exact', 'rdm_fid.dat' w p t 'numerics'
! or simply: plot 'rdm_fid.dat' w p
do  d = 2, 30, 2 ;   allocate( rsv(1:d), rdm(1:d,1:d) ) ;   avgf = 0.d0
  do j = 1, ns
    call rsvg(optg, d, rsv) ;   call rdmg(optg, d, rdm)
    avgf = avgf + fidelity_pm(d, rsv, rdm)
  enddo
